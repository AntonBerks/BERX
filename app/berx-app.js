...
function mapStoryToSpatial(story, ownerGuid, placement = {}) {
  const object = baseObject("moment", `story-${story.id}`, story.caption || "\u0418\u0441\u0442\u043E\u0440\u0438\u044F", String(story.id), 0.5, placement, {
    ...
    ...story.time_expires !== void 0 ? { endsAt: story.time_expires } : {}
  });
  return { object, media: [], relations: [ownedBy(object.id, ownerGuid)] };
}
function mapMemoryToSpatial(memory, viewerGuid, placement = {}) {
...
function mapTripToSpatial(trip, placement = {}) {
  ...
  const relations = [ownedBy(object.id, trip.owner_guid)];
...
function mapNotificationToSpatial(notification, placement = {}) {
...
async function loadBerxWorld(api2, options = {}) {
  ...
  await Promise.all([
     ...
     * keeps its world.
     */
    attempt("stories", () => api2.storiesFeed(), (response) => {
    ...
    attempt("trips", () => api2.trips(), (response) => {
      for (const trip of response.trips) entries.push(toEntry(mapTripToSpatial(trip)));
    }),
    attempt("notifications", () => api2.notifications(), (response) => {
  ...
  if (viewerId) {
    ...
    if (Number.isFinite(viewerGuid)) {
      await attempt("memories", () => api2.memories(), (response) => {
      ...
      switch (action) {
        ...
        case "save": {
          if (object.kind !== "place" && object.kind !== "business") {
            throw new Error(`BERX: \xAB${action}\xBB \u043F\u043E\u043A\u0430 \u043D\u0435\u0442 \u043D\u0430 \u0441\u0435\u0440\u0432\u0435\u0440\u0435 \u0434\u043B\u044F \xAB${object.kind}\xBB`);
          }
