...
if (isset($owner) && !empty($owner)) {
    // Block enforcement — nothing here previously stopped a blocked
    // user from posting straight onto their blocker's wall via
    // wallowner (found in cross-module security audit). Same silent-
    // failure pattern as message send: no distinguishing error, just
    // refuse the post as if it failed for any other reason.
    if (com_is_active('OssnBlock')) {
        $poster_user = ossn_loggedin_user();
        $owner_user = ossn_user_by_guid($owner);
        if ($poster_user && $owner_user && OssnBlock::isBlocked($poster_user, $owner_user)) {
