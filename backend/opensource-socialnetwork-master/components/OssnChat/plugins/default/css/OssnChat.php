:root{
    ...
    --ossn-inchat-icon-color: var(--berx-text-dim, #a3a3a3);
}
.ossn-chat-base {
    border-bottom: 0;
    bottom: 0px;
...
.friend-tab-item .tab-container {
    ...
    border-top-left-radius: 8px;
    border-top-right-radius: 8px;    
    box-shadow: var(--berx-shadow, 0 12px 28px rgba(0,0,0,0.35));
}
...
/* ============================================================
   BERX CHAT BAR
   OssnChat renders a persistent footer chat bar on every page
   (extends 'ossn/page/footer'), so its leftover light surfaces
   were visible product-wide — except on /messages, where
   OssnMessages deliberately unextends it.
   Appended as overrides rather than rewriting the original rules,
   so all OssnChat JS behaviour and class toggling is untouched.
   ============================================================ */
.ossn-chat-tab,
.friends-list .ossn-chat-tab-titles,
.ossn-chat-windows-long {
...
.ossn-chat-windows-long {
	border-left: 1px solid var(--berx-border-soft, rgba(255,255,255,0.08)) !important;
}
.ossn-chat-windows-long .inner {
	border-top: 1px solid var(--berx-border-soft, rgba(255,255,255,0.08));
}
...
.ossn-chat-windows-long .friends-list-item:hover {
	background: var(--berx-glass-2, rgba(255,255,255,0.07));
	border-top-color: var(--berx-border-soft, rgba(255,255,255,0.08));
	border-bottom-color: var(--berx-border-soft, rgba(255,255,255,0.08));
}
...
.ossn-chat-tab img,
.ossn-chat-windows-long img {
	border-radius: 50%;
	object-fit: cover;
}

/* ---------- BERX pass-3 sweep: two rules a project-wide grep found
   still light after pass 1 ---------- */
.friend-tab-item .friend-tab {
...
.friend-tab-item .friend-tab input[type='text']::placeholder {
	color: var(--berx-text-faint, #6f6f6f);
}
.ossn-chat-tab-close {
	color: var(--berx-text-faint, #6f6f6f) !important;
}
.ossn-chat-tab-close:hover {
	color: var(--berx-white, #fff) !important;
}
