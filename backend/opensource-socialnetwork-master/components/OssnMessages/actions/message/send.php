...
// Block enforcement — core OSSN had no block check on message send at
// all (found in end-to-end audit): a blocked user could keep
// messaging through the normal messenger even after being blocked.
// Same 404-style silent failure as the rest of BERX's block handling
// — a blocked sender gets no distinguishing error, just the same
// generic "0" the "user doesn't exist" branch above already returns.
if (com_is_active('OssnBlock')) {
	$sender_user = ossn_loggedin_user();
	$recipient_user = ossn_user_by_guid($to);
	if ($sender_user && $recipient_user && OssnBlock::isBlocked($sender_user, $recipient_user)) {
		echo 0;
		exit;
	}
}
