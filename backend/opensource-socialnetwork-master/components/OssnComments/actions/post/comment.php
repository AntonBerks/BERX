...
// Block enforcement — nothing here previously stopped a blocked user
// from commenting on their blocker's content at all (found in
// cross-module security audit). Resolve the commented-on entity's
// owner via the generic ossn_get_entity() helper (works across post
// types this component serves — wall posts, group posts, etc.) and
// refuse silently, same pattern as message send / wall post.
if (com_is_active('OssnBlock')) {
	$entity = ossn_get_entity($post);
