<?php
/**
 * BERX admin dashboard.
 * Theme override of system/plugins/default/pages/administrator/contents/dashboard.
 * Copied verbatim except for the inline light-background <style> block
 * noted inline below. All data queries, chart ids, XHR callbacks and
 * action URLs are unchanged.
 */
?>
<?php
/**
...
<?php /* BERX: the stock page injected an inline
   <style>body{background:#fdfdfd;}</style> here. Being an inline
   <style> late in the document, it beat the admin stylesheet on
   equal specificity and forced a near-white body on the dashboard
   only — the one admin page that looked unmistakably stock OSSN.
   Removed; the body background now comes from the BERX admin
   stylesheet like every other admin page. Nothing else in this
   file is changed: the same chart.js loads, the same
   OssnUser::genderTypes() data, the same cache-flush action URL,
   the same #users-count-graph / #usercount-lineLegend canvas ids
   and the same three dynamic javascript views below. */ ?>
