...
/* ============================================================
   BERX ADMIN — dark operational workspace
   Appended as overrides. No admin markup, route, form, action or
   JavaScript is modified: every administrative capability (users,
   components, themes, settings, moderation, logs) behaves exactly
   as before. This restyles the shell those tools live in, which
   was still 100% original OSSN light chrome.
   Anchors are the real admin classes: .header, .header-dropdown,
   .topbar-menu, .ossn-layout-admin, .contents, .page-title,
   .table-titles, .ossn-users-list.
   ============================================================ */
body,
.ossn-layout-admin {
	background: var(--berx-black, #050505);
...
.page-title {
	...
	border-bottom: 1px solid var(--berx-border-soft, rgba(255,255,255,0.08));
	padding-bottom: var(--berx-space-3, 12px);
	margin-bottom: var(--berx-space-4, 16px);
}

/* Operational tables: dense, scannable, dark. */
.ossn-layout-admin table,
.ossn-users-list {
	width: 100%;
	border-collapse: separate;
	border-spacing: 0;
...
.ossn-layout-admin table td,
.ossn-layout-admin table th {
	border-bottom: 1px solid var(--berx-border-soft, rgba(255,255,255,0.08)) !important;
	padding: var(--berx-space-3, 12px) !important;
	vertical-align: middle;
}
.ossn-layout-admin table tr:hover td {
	background: rgba(255,255,255,0.03);
}
.ossn-layout-admin table img {
	border-radius: 50%;
	object-fit: cover;
}

/* Admin forms inherit the same field language as the front end. */
.ossn-layout-admin input[type="text"],
.ossn-layout-admin input[type="email"],
.ossn-layout-admin input[type="password"],
.ossn-layout-admin input[type="number"],
.ossn-layout-admin select,
.ossn-layout-admin textarea {
	background: var(--berx-glass-2, rgba(255,255,255,0.07)) !important;
...
.ossn-layout-admin .ossn-widget {
	background: var(--berx-glass-2, rgba(255,255,255,0.07));
	border: 1px solid var(--berx-border-soft, rgba(255,255,255,0.08));
	border-radius: var(--berx-radius, 16px);
}
.ossn-layout-admin .widget-heading {
	color: var(--berx-white, #fff);
	border-bottom: 1px solid var(--berx-border-soft, rgba(255,255,255,0.08));
	font-weight: 700;
}
...
@media (max-width: 991px) {
	/* The admin shell was desktop-only; make tables scrollable rather
	   than letting them force horizontal page scroll on a phone. */
	.ossn-layout-admin .contents {
...
/* ============================================================
   BERX ADMIN — DEEP PASS
   The shell was darkened in pass 1, but the pages inside it were
   still stock white. Each rule below was located by inspecting the
   specific admin page/markup it belongs to, not by bulk color
   replacement. Every admin route, form, action and JS binding is
   untouched — this is presentation only.
   ============================================================ */

/* Page container + every admin <form> (was `background:#ffffff`
   applied to the bare `form` element, so EVERY admin form was a
   white sheet regardless of page). */
.ossn-admin-pg-content,
.ossn-layout-admin form,
form.ossn-form {
...
/* Focus ring: was a light #ddd border + #eee glow, invisible on dark. */
...
/* Was #87909e — a mid grey that reads as "disabled" for the primary
   metric on the page. The number is the point; make it the brightest
   thing in the card. */
.ossn-admin-dashboard .card-value {
	color: var(--berx-white, #fff);
}
...
/* ---------- Users list / moderation tables ---------- */
.ossn-admin-all-users {
	background: transparent !important;
	border-color: var(--berx-border-soft, rgba(255,255,255,0.08)) !important;
	color: var(--berx-text, #f2f2f2);
}
.ossn-admin-all-users .user-avatar-modern {
	object-fit: cover;
...
.ossn-admin-search-pro-line input {
	background: transparent !important;
	border: none !important;
	color: var(--berx-text, #f2f2f2) !important;
}

/* ---------- System message + loading surfaces ---------- */
.ossn-message-box {
...
/* Any remaining light chip/box borders inside admin pages. */
.ossn-layout-admin [class*="box"],
.ossn-layout-admin .panel,
.ossn-layout-admin .well {
	border-color: var(--berx-border-soft, rgba(255,255,255,0.08)) !important;
}

/* Mobile: the admin dashboard grid was desktop-first. */
@media (max-width: 767px) {
	.ossn-admin-dashboard .graph-card-main { height: auto; }
	.ossn-admin-dashboard .metric-card,
	.ossn-admin-dashboard .cache-action-card { height: auto; }
	.ossn-admin-search-pro-line { flex-wrap: wrap; }
}
