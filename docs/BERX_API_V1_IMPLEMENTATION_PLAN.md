...
### Wave 2 — новые лёгкие классы + REST-обёртка над OssnGroup: CLOUD-STATIC-VERIFIED, NOT RUNTIME-VERIFIED

Новые классы: `OssnPoints` (баланс/уровень/история + реальный дневной
streak), `OssnReport` (очередь модерации), `OssnDating` (Match),
`OssnStories` (24ч истории, включая Event Story Wall). Новый REST-файл
без нового класса: `communities.php` поверх уже реального `OssnGroup`.
Одна новая миграция: `1785170300.php` — ALTER `ossn_points_balance`
(добавляет `current_streak`/`longest_streak`/`last_active_date`;
`ossn_points_balance`/`ossn_points_log` сами по себе уже существовали).

**Реально отсутствующая функция (core, не BERX):** `ossn_messagesearch_query()`
из Wave 1 — уже задокументировано там.

**Сознательно не реализовано (реальный, раскрытый пробел, не
подделка):** `POST /dating/boost` — в реальной, уже смигрированной
`ossn_dating_profiles` нет колонки `boosted_until`, которую ожидает
`client.ts`. Требует отдельной проверенной миграции и решения, как
буст должен влиять на `discover()` — не стал добавлять второпях.
Загрузка фото для Dating (`ossn_dating_photos`) тоже не реализована в
этом REST-файле — в `client.ts` для неё вообще нет метода, добавлять
эндпоинт, до которого нечему достучаться, значило бы нарушить
собственное правило проекта "клиентский метод и PHP-эндпоинт
появляются в один день".

**Найдены и исправлены 2 реальные ошибки в собственном черновике до
коммита (не в уже существующем коде — в новых файлах этой волны):**
1. `communities.php` вызывал `$model_isModerator(...)` — опечатка,
   PHP интерпретировал бы это как вызов функции по имени из
   несуществующей переменной (fatal `Call to undefined function`).
   Исправлено на реальный `$group->isModerator($guid)`.
2. Черновик `OssnDating::excludedGuids()`/`discover()` содержал
   вызовы `OssnBlock::class_exists_hint()` (мёртвый, ничего не
   значащий код) и `OssnBlock::isBlockedGuids()` (метод, которого не
   существует). Исправлено на тот же паттерн `stdClass`-обёрток с
   `->guid`, что и в `ossn_api_is_blocked()` (Wave 1) — с явным
   docblock-примечанием, почему сырые guid туда передавать нельзя.

Схема `OssnDating::addStory()`-эквивалента для событийных историй
(`OssnStories::addStory($eventGuid=...)`) зависит от нового метода
`OssnEvents::isRealAttendee($eventGuid, $userGuid)`, которого пока не
существует — обёрнуто в `class_exists('OssnEvents')`, безопасный
no-op до Wave 3, контракт метода явно задокументирован в самом коде
для будущей реализации.

Проверки — тот же протокол: `php -l` на всё (1185 файлов, 0 ошибок),
systematic-грап на `count()/array_merge()` на результатах
`select(..., true)` и на коллизии имён функций/классов по всем новым
файлам волны — чисто после исправлений выше.
