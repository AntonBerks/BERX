...
const person = {
  ...
  sourceId: 'test',
  transform: { position: { x: 0, y: 0, z: 0 }, scale: { x: 1, y: 1, z: 1 }, rotation: { x: 0, y: 0, z: 0 } },
...
export function assertBerx5DRuntimeInvariants(): void {
  ...
  const beforeEnter = runtime.worldState.camera;
  runtime.enterWorld({ id: 'detail', sourceRoute: '/profile/test', focusObjectId: person.id, enteredAt: Date.now() });
  runtime.frame(0.02);
  ...
  runtime.frame(0.02);
  if (runtime.canGoBack) throw new Error('5D invariant: back navigation left stale history');

  const restored = runtime.worldState.camera;
  if (Math.abs(restored.position.x - beforeEnter.position.x) > 0.001 || Math.abs(restored.position.y - beforeEnter.position.y) > 0.001 || Math.abs(restored.position.z - beforeEnter.position.z) > 0.001) {
    throw new Error('5D invariant: back camera did not restore the pre-entry pose');
  }
  if (Math.abs(restored.target.x - beforeEnter.target.x) > 0.001 || Math.abs(restored.target.y - beforeEnter.target.y) > 0.001 || Math.abs(restored.target.z - beforeEnter.target.z) > 0.001) {
    throw new Error('5D invariant: back camera did not restore the pre-entry target');
  }
