 ...
 *
 * BERX ICON SYSTEM — one family, one geometry.
 *
 * Source: Lucide (ISC, commercially usable — see
 * design-resources/icons/LICENSE-lucide.txt and the license registry).
 * The 2050-glyph library is NOT shipped; 82 glyphs the product actually
 * uses were curated, then normalised so every one shares the same
 * canvas, stroke width, caps and joins. All 82 passed that check with
 * zero deviations, so this is one system rather than a pile of icons.
 *
...
export type BerxIconName = keyof typeof BERX_ICON_GEOMETRY;

/** Raw geometry, normalised at build time from the curated Lucide set. */
export const BERX_ICON_GEOMETRY: Record<string, string> = {
