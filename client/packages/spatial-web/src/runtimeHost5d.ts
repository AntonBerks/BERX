/** BERX MAX 5D web host: real GPU scene + camera input + spatial picking. */
import { Berx5DRuntime, pickSpatialObject, type BerxSpatialObject } from '@berx/spatial';
import { BerxThreeRuntimeRenderer } from './threeRuntime';

export interface Berx5DWebHostOptions { canvas?:HTMLCanvasElement; reducedMotion?:boolean; deviceMotion?:boolean; pixelRatioCap?:number; transitionDuration?:number; }
...
export function createBerx5DWebHost(options:Berx5DWebHostOptions={}):Berx5DWebHost{
 ...
 const renderer=new BerxThreeRuntimeRenderer(canvas);const pixelRatioCap=Math.max(1,options.pixelRatioCap??2);
 ...
 const resize=()=>{const rect=canvas.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,pixelRatioCap),width=Math.max(1,Math.round(rect.width*dpr)),height=Math.max(1,Math.round(rect.height*dpr));if(canvas.width!==width||canvas.height!==height){canvas.width=width;canvas.height=height;renderer.resize(width,height);}};
 const frame=(now:number)=>{if(!running)return;const dt=Math.min(0.05,Math.max(0,(now-last)/1000));last=now;resize();renderer.sync(runtime.frame(dt));raf=requestAnimationFrame(frame);};
 ...
 const onPointerMove=(e:PointerEvent)=>{if(!dragging)return;const dx=e.clientX-lastX,dy=e.clientY-lastY;lastX=e.clientX;lastY=e.clientY;runtime.input({panX:-dx*0.018,panY:dy*0.018,depthDelta:0,pinch:0});};
 const onPointerUp=(e:PointerEvent)=>{if(!dragging)return;dragging=false;canvas.releasePointerCapture?.(e.pointerId);if(Math.hypot(e.clientX-downX,e.clientY-downY)>8)return;const rect=canvas.getBoundingClientRect(),ndcX=((e.clientX-rect.left)/Math.max(1,rect.width))*2-1,ndcY=((e.clientY-rect.top)/Math.max(1,rect.height))*2-1;const frameState=runtime.frame(0);const hit=pickSpatialObject(frameState,ndcX,ndcY,rect.width/Math.max(1,rect.height));if(hit)runtime.focus(hit.id);};
 ...
 const onDeviceMotion=(e:DeviceOrientationEvent)=>{if(reducedMotionPreference())return;runtime.input({panX:0,panY:0,depthDelta:0,pinch:0,motion:{pitch:(e.beta??0)/45,roll:(e.gamma??0)/45,yaw:(e.alpha??0)/180,intensity:0.65}});};
