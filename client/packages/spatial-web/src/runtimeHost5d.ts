/** BERX MAX 5D web host — real GPU scene, camera input, picking and world navigation. */
...
export function createBerx5DWebHost(options:Berx5DWebHostOptions={}):Berx5DWebHost{
 ...
 const runtime=new Berx5DRuntime({reducedMotion:options.reducedMotion??reducedMotionPreference(),deviceMotionEnabled:options.deviceMotion!==false});
 ...
 const frame=(now:number)=>{if(!running)return;const dt=Math.min(.05,Math.max(0,(now-last)/1000));last=now;resize();renderer.render(runtime.frame(dt));raf=requestAnimationFrame(frame);};
 ...
 const onPointerUp=(e:PointerEvent)=>{if(!dragging)return;dragging=false;canvas.releasePointerCapture?.(e.pointerId);if(Math.hypot(e.clientX-downX,e.clientY-downY)>8)return;const rect=canvas.getBoundingClientRect(),dpr=canvas.width/Math.max(1,rect.width),frameState=runtime.frame(0),hit=renderer.pick(frameState,(e.clientX-rect.left)*dpr,(e.clientY-rect.top)*dpr);if(hit)runtime.focus(hit.objectId);};
 ...
 const onDeviceMotion=(e:DeviceOrientationEvent)=>{if(reducedMotionPreference())return;runtime.input({panX:0,panY:0,depthDelta:0,pinch:0,motion:{pitch:(e.beta??0)/45,roll:(e.gamma??0)/45,yaw:(e.alpha??0)/180,intensity:.65}});};
 ...
 return {canvas,runtime,renderer,addObject:object=>runtime.registerObject(object),removeObject:id=>runtime.world.removeObject(id),focus:id=>{const before=runtime.world.getObject(id);if(!before)return false;runtime.focus(id);return true;},enterWorld:(id,sourceRoute,destination)=>runtime.enterWorld({id,sourceRoute,enteredAt:Date.now()},destination),back:()=>{const before=runtime.worldState;runtime.back();return before.id!==runtime.worldState.id;},start,stop,destroy:()=>{stop();canvas.removeEventListener('pointerdown',onPointerDown);canvas.removeEventListener('pointermove',onPointerMove);canvas.removeEventListener('pointerup',onPointerUp);canvas.removeEventListener('pointercancel',onPointerUp);canvas.removeEventListener('wheel',onWheel);canvas.removeEventListener('touchstart',onTouchStart);canvas.removeEventListener('touchmove',onTouchMove);canvas.removeEventListener('touchend',onTouchEnd);window.removeEventListener('resize',onResize);if(options.deviceMotion!==false&&'DeviceOrientationEvent' in window)window.removeEventListener('deviceorientation',onDeviceMotion);renderer.dispose();if(!options.canvas)canvas.remove();}};
}
