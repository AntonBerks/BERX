...
function RouteRenderer({name, params}: {name: BerxRouteName; params: unknown}) {
	...
	switch (name) {
		...
		case 'Profile':
			return (
				<ProfileScreen
					...
					onOpenRecentCheckins={!nav.canGoBack ? () => nav.push('RecentCheckins', undefined) : undefined}
		...
		case 'RecentCheckins':
			return <RecentCheckinsScreen api={api} onOpenPlace={(guid) => nav.push('PlaceDetail', {guid})} onBack={nav.pop} />;
		case 'Saved':
...
function NowScreenRoute({onOpenProfile}: {onOpenProfile: (username: string) => void}) {
	...
	return (
		<NowScreen
			...
			onOpenMyProfile={() => nav.push('Profile', {username: undefined})}
			onOpenPlaces={() => nav.push('Places', undefined)}
			onOpenEvents={() => nav.push('Events', undefined)}
			onCreatePost={() => nav.push('CreatePost', undefined)}
			onOpenStoryGroup={(group) => {
